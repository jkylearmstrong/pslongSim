...
`MLPScore` has no dependency on `pslongSim` in any DESCRIPTION field.
The graph is acyclic by construction and guarded by
...
`fit_msm_gee_cont()`, `fit_msm_gee_bin()`, `fit_msm_cox()`) so
simulation sweeps are self-contained without `MLPScore` installed. These
are benchmarking baselines — for advanced estimation (cross-fitting,
TWANG, AIPW, diagnostics) use `MLPScore`.
...
## Non-Linear & Longitudinal Confounding Features

### Outcome and Adherence Non-Linearities

When `non_linear_feature = TRUE`, interaction terms (e.g.,
`Age * SideEffect`) are added to the outcome linear predictor.
Similarly, non-linear terms can be injected into the adherence model
linear predictor using `nonlin_coefs_adherence`:
...
dat <- generate_longitudinal_data(
  ...
  nonlin_coefs_adherence = c("step(Age, 65):SideEffect" = 0.25)
)
```

### Basis-Breaking Non-Linear Forms

`pslongSim` supports flexible non-linear terms that are tree-friendly
but linear-hostile: - **Multi-way interactions**:
`Age:SideEffect:Biomarker` - **Step thresholds**:
`step(Age, 65):SideEffect` - **Quadratics / U-shapes**: `I(Age^2)` -
**Logic regions**: `xor(SideEffect, Compliant)`

### Treatment-Confounder Feedback

To model longitudinal confounding where past treatment compliance
influences future time-varying covariates, set `comp_feedback` in
`se_model` or `biom_model`:

``` r
dat <- generate_longitudinal_data(
  demo, num_visits = 6, trt_map = trt_map,
  se_model = list(intercept = -1.2, lag_se = 1.0, comp_feedback = 0.4),
  biom_model = list(intercept = 0, phi = 0.75, comp_feedback = 0.5)
)
```
